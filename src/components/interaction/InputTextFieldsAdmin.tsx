...
const InputTextFieldsAdmin: FC<Props> = (props: Props) => {
    ...
    return (
        <Box component="form" sx={{ '& .MuiTextField-root': { m: 1, width: devices.isDesktop ? "400px" : devices.isTablet ? "400px" : devices.isMobile ? "90%" : "90%" },}} noValidate autoComplete="off">
            <div>
                <TextField required id="outlined-required" label="Paketnamn" inputProps={{ maxLength: 35 }} onChange={(event) => {props.setNameProp(event.target.value)}} value={props.nameInputValue} error={props.updatedErrorList[0].error} /><br />
                <TextField required id="outlined-multiline-static" label="Paketbeskrivning" multiline rows={4} inputProps={{ maxLength: 200 }} onChange={(event) => {props.setDescProp(event.target.value)}} value={props.descInputValue} error={props.updatedErrorList[1].error}/><br />
